  ...
  <div class="wrap">
    <h1>testing</h1>
      ...
      if(isset($_POST['des_data'])){
        ...
        destroy_data();
        echo 'All dynamic data destroyed and database recreated, refresh page to recreate base dummy data';
      }
        ...
        echo '<form name="post" action="' . admin_url( 'admin.php?page=pilot_test' ) . '" method="post"> <input type="hidden" name="des_data" value="true"> <input type="submit" value="Reset db and destroy dynamic users" ></form>';
      }
      else{
        create_data_base();
...
function data_exist(){
  global $wpdb;
  $result = $wpdb->get_results("SELECT * FROM wp_School_answer");
  if(count($result) == 0)
    return false;
  else
    return true;
...
function school_answers_exist($school_id, $company_id){
  global $wpdb;
  $result = $wpdb->get_results("SELECT * FROM wp_School_answer WHERE school_id=$school_id AND company_id=$company_id");
  if(count($result) == 23)
    return true;
  else
    return false;
...
function company_answers_exist($company_id){
  global $wpdb;
  $result = $wpdb->get_results("SELECT * FROM wp_Company_answer WHERE company_id=$company_id");
  if(count($result) == 23)
    return true;
  else
    return false;
...
function create_data_dynamic(){
  ...
  global $wpdb;
  $wpdb->query( "INSERT INTO wp_Company_answer VALUES
...
function destroy_data(){
  ...
  // destroy generic school accounts
  $schools = get_users( array('role' => 'school') );
  ...
  // destroy generic company accounts
  $companies = get_users( array('role' => 'company') );
  for($i=0; $i<count($companies); $i++){
